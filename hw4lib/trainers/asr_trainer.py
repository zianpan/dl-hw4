...
class ASRTrainer(BaseTrainer):
    ...
    def _train_epoch(self, dataloader):
        ...
        for i, batch in enumerate(dataloader):
            ...
            feats, targets_shifted, targets_golden, feat_lengths, transcript_lengths = [item.to(self.device) for item in batch]
    ...
    def recognize(self, dataloader, recognition_config: Optional[Dict[str, Any]] = None, config_name: Optional[str] = None, max_length: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        with torch.inference_mode():
            for i, batch in enumerate(dataloader):
                ...
                feats, _, targets_golden, feat_lengths, _ = [item.to(self.device) if item is not None else item for item in batch]
                ...
                if recognition_config['beam_width'] > 1:
                    ...
                    temperature=recognition_config['temperature'], repeat_penalty=recognition_config['repeat_penalty'])
                    # Pick best beam
                    seqs = seqs[:, 0, :]
                    scores = scores[:, 0]
                else:
