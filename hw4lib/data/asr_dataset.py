...
class ASRDataset(Dataset):
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        batch_feats  = [feat.permute(1, 0) for feat, shifted, golden in batch]  # feat from (F, T) to (T, F)
        ...
        feat_lengths_list = [len(feat) for feat in batch_feats]
        feat_lengths = torch.tensor(feat_lengths_list)  # B
        ...
        if self.partition != "test-clean":
            ...
            batch_shifted      = [shifted for feat, shifted, golden in batch]  # B x T
            batch_golden       = [golden for feat, shifted, golden in batch]  # B x T
