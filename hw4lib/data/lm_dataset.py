...
class LMDataset(Dataset):
    ...
    def __init__(
        ...
        self.text_dir = os.path.join(self.root, partition)
        ...
        # TODO: Take subset
        subset_size = 1
        ds_len = len(self.text_files)
        self.text_files = self.text_files[:int(ds_len*subset_size)]
    ...
    def collate_fn(self, batch: List[Tuple[torch.LongTensor, torch.LongTensor]]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        shifted_transcripts = list(shifted_transcripts)
        golden_transcripts = list(golden_transcripts)
